...
class HtmlTreeBuilder extends TreeBuilder {
    ...
    Element insert(Token.StartTag startTag) {
    	...
    	if (startTag.isSelfClosing() && !Tag.isKnownTag(startTag.name())) {//SAEG - FAULT
        //if (startTag.isSelfClosing()) {//SAEG - FIXING
            Element el = insertEmpty(startTag);
            process(new Token.EndTag(el.tagName()));//SAEG - FAULT
            /*stack.add(el);//SAEG - FIXING
            tokeniser.emit(new Token.EndTag(el.tagName())); */ // ensure we get out of whatever state we are in. emitted for yielded processing //SAEG - FIXING
            return el;
        }
